...
class LaplaceNode(ASTNode):
    ...
    def __repr__(self) -> str:
        return f'Laplace(name: {self.name}, node: {self.node_name}, loc: {self.loc_name}, scale: {self.scale_name})'
...
class Laplace(VModule):
    def __init__(self, size, loc, scale, name: str):
        ...
        terminal_node = LaplaceNode(node_name=node_name, loc_name=loc.name, scale_name=scale.name, name=name)
