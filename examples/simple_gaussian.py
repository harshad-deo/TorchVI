...
class Model(nn.Module):
    ...
    def forward(self, xs, device):
        mu, prior_contrib, jac_contrib, entropy_contrib = self.x(device)
        dist = distributions.Normal(mu, self.scale_known)
        lp = dist.log_prob(xs).sum()

        return lp + prior_contrib + jac_contrib + entropy_contrib

    def sample(self, size, device):
        mu_samp = torch.squeeze(self.x.sample(size, device))
        dist = distributions.Normal(mu_samp, self.scale_known)
        return dist.sample()
...
if __name__ == "__main__":
    ...
    num_epochs = 200
    model, losses, actual_samples = fit(sigma_known, xs, num_epochs)
    ...
    expected_samples = np.random.normal(loc=mu_known, scale=sigma_known, size=num_samples * 10)
